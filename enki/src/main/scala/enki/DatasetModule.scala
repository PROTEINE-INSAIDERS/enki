...
trait DatasetModule extends OptionColumnRelation {
  ...
  implicit class DatasetExtensions[T](val dataset: Dataset[T]) {
    ...
    //TODO: параметры операции cast.
    // 1. Используем ли метаданные при определении схемы?
    // 2. Отбрасываем ли отсутсвующие в схеме колонки?
    // 3. Допускаем ли усечение данных?

    def cast[R: TypeTag](strict: Boolean): Dataset[R] = {
